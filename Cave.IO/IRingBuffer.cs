...
public interface IRingBuffer<TValue> : IRingBufferCursor<TValue>
{
    ...
    /// <remarks>This is an unsorted dump of the current buffer state. Be aware that the state may have changed on completion already.
    /// There are no checks or race condition preventions at this function!</remarks>
    ...
    /// <remarks>
    ///     <para>
    ///         Multitread read (fifo queue) function:
    ///         This blocks until an item can be read. Any thread waiting for an item can be activated, there is no activation order for threads
    ///         waiting for an item.
    ///     </para>
    ///     <para>
    ///         This function uses a global cursor. 
    ///         All items read by this function can no longer be read by cursors!
    ///         Multiple threads can read but each item can only be read once. If you need multiple readers
    ///         reading all items of the ring buffer, use <see cref="GetCursor"/> and do not use this function.
    ///     </para>
    /// </remarks>
    new TValue Read();
    ...
    ///     <para>
    ///         Multitread read (fifo queue) function:
    ///         If multiple threads read from the buffer or the buffer does not contain enough items, 
    ///         this will return less than <paramref name="count"/> items.
    ///     </para>
    ///     <para>
    ///         This function uses a global cursor. 
    ///         All items read by this function can no longer be read by cursors!
    ...
    ///     <para>
    ///         Multitread read (fifo queue) function:
    ///         Since the ringbuffer is lockless the first thread seeing the item will get it.
    ///     </para>
    ///     <para>
    ///         This function uses a global cursor. 
    ///         All items read by this function can no longer be read by cursors!
